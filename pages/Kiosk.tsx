...
const KIOSK_THEMES: Record<KioskTheme, {
  ...
  'dark-neon': {
    ...
    blob1: 'bg-cyan-500/25', // Ambient glow - Cyan
    blob2: 'bg-blue-600/20', // Ambient glow - Blue
    accent: 'text-cyan-400', // Primary accent - Cyan
    text: 'text-white', // Pure white for headings
    subText: 'text-slate-300', // Light gray for body text (#94a3b8)
    inputBg: 'bg-white/5', // Glassmorphism input
    inputBorder: 'border-cyan-500/40', // Neon border
    glowFrom: 'from-cyan-400', // Gradient start - Cyan
    glowTo: 'to-blue-500', // Gradient end - Blue
    isDark: true
  },
  'dark-gradient': {
...
const Kiosk: React.FC = () => {
  ...
  // Get current theme for screensaver
  ...
  const screensaver = screensaverActive && (
    <div 
      className={`fixed inset-0 z-[200] overflow-hidden flex flex-col items-center justify-center transition-all duration-500 ${currentTheme.bg}`} 
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className={`absolute top-[-10%] left-[15%] w-[600px] h-[600px] ${currentTheme.blob1} rounded-full blur-[180px] animate-blob`}></div>
        <div className={`absolute bottom-[-10%] right-[20%] w-[500px] h-[500px] ${currentTheme.blob2} rounded-full blur-[160px] animate-blob animation-delay-2000`}></div>
        <div className={`absolute top-[40%] left-[60%] w-[400px] h-[400px] ${currentTheme.blob1} rounded-full blur-[140px] animate-blob animation-delay-4000`}></div>
        {currentTheme.isDark && (
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full h-full p-8">
        ...
        <div className={`absolute top-6 left-6 ${currentTheme.isDark ? 'bg-black/40' : 'bg-white/60'} px-5 py-3 rounded-2xl backdrop-blur-sm border ${currentTheme.isDark ? 'border-white/10' : 'border-black/10'} z-20`}>
          <div className={`text-2xl md:text-3xl font-mono ${currentTheme.text} font-bold`}>
        ...
        {settings?.headerImage && (
          <img src={settings.headerImage} alt="header" className={`absolute top-6 right-6 max-h-14 md:max-h-16 drop-shadow-2xl rounded-xl border ${currentTheme.isDark ? 'border-white/10 bg-white/10' : 'border-black/10 bg-black/5'} animate-fade-in z-20`} />
        )}
        ...
        {settings?.screensaverCustomText?.enabled && 
         ...
         settings.screensaverCustomText.position === 'top' && (
          <div className={`
            ...
            font-bold ${currentTheme.text} drop-shadow-2xl font-serif
          `}>
            {settings.screensaverCustomText.text}
          </div>
        )}
        ...
        <div className="text-center max-w-4xl mx-auto flex flex-col items-center justify-center mt-16">
          ...
          {settings?.screensaverImages && settings.screensaverImages.length > 0 && (
            <div className={`relative w-[75vw] max-w-[650px] aspect-video rounded-3xl overflow-hidden shadow-2xl border-2 ${currentTheme.isDark ? 'border-white/20' : 'border-black/10'}`}>
                  ...
                  {settings.screensaverImages.map((_, idx) => (
                    <div 
                      ...
                      className={`w-2 h-2 rounded-full transition-all duration-300 ${
                        idx === screensaverIndex 
                          ? `${currentTheme.isDark ? 'bg-white' : 'bg-gray-800'} w-6` 
                          : `${currentTheme.isDark ? 'bg-white/40' : 'bg-gray-800/40'}`
                      }`} 
                    />
                  ))}
                </div>
          ...
          {settings?.screensaverPhrases && settings.screensaverPhrases.length > 0 && (
            <div className="relative h-20 md:h-24 flex items-center justify-center overflow-visible mt-12 mb-4">
              {settings.screensaverPhrases.map((phrase, idx) => (
                <p 
                  key={idx}
                  className={`absolute text-2xl md:text-4xl font-bold transition-all duration-700 ease-in-out whitespace-nowrap ${
                    idx === phraseIndex 
                ...
                <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {settings.screensaverPhrases.map((_, idx) => (
                    <div 
                      ...
                      className={`w-2 h-2 rounded-full transition-all duration-300 ${
                        idx === phraseIndex 
                          ? `${currentTheme.accent.replace('text-', 'bg-')}` 
                          : `${currentTheme.isDark ? 'bg-white/20' : 'bg-black/20'}`
                      }`} 
                    />
                  ))}
                </div>
          ...
          {settings?.screensaverCustomText?.enabled && 
           ...
           settings.screensaverCustomText.position === 'center' && (
            <h1 className={`
              mt-6 font-bold ${currentTheme.text} drop-shadow-2xl font-serif animate-fade-in
        ...
        <div className={`absolute bottom-4 right-4 ${currentTheme.subText} text-xs ${currentTheme.isDark ? 'bg-black/40' : 'bg-white/60'} px-3 py-1.5 rounded-lg backdrop-blur-sm border ${currentTheme.isDark ? 'border-white/10' : 'border-black/10'} z-20`}>
          المس الشاشة للخروج
        </div>
