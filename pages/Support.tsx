...
import { Activity, CheckCircle, Server, RefreshCw, Power, Database, Cloud, HardDrive, ToggleLeft, ToggleRight, Save, ShieldCheck, AlertTriangle, XCircle, Megaphone, Send, Loader2, Image, Palette, X, Check, Headset, MessageCircle, Instagram, Link, Building, UserCircle } from 'lucide-react';

// ═══════════════════════════════════════════════════════════════
// 🎨 Theme configurations - Futuristic Glass & Dark Neon Theme
// ═══════════════════════════════════════════════════════════════
const THEME_CONFIG: Record<string, { name: string; nameEn: string; emoji: string; gradient: string; colors: string[] }> = {
    default: { name: 'نيون سايبر', nameEn: 'Cyber Neon', emoji: '⚡', gradient: 'from-cyan-500 to-blue-600', colors: ['bg-cyan-500', 'bg-blue-600'] },
    ...
    violet: { name: 'بنفسجي ملكي', nameEn: 'Royal Violet', emoji: '✨', gradient: 'from-violet-500 to-purple-600', colors: ['bg-violet-500', 'bg-purple-600'] },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-4">
          <div>
            <h2 className="text-4xl font-bold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-300">لوحة تحكم الدعم الفني</h2>
            <p className="text-slate-400 flex items-center gap-2">
              <span className="w-2 h-2 bg-cyan-500 rounded-full animate-pulse"></span>
              مراقبة النظام، التشخيص الذكي، وإدارة البث والإعلانات
            </p>
          </div>
          <div className="flex gap-3">
              <button onClick={fetchSystemStats} className="p-3 bg-cyan-500/5 border border-cyan-500/20 rounded-xl hover:bg-cyan-500/10 hover:border-cyan-500/30 text-slate-300 hover:text-cyan-400 transition-all" title="تحديث الإحصائيات">
                  <RefreshCw className="w-5 h-5" />
              </button>
              <button onClick={() => window.location.reload()} className="p-3 bg-cyan-500/5 border border-cyan-500/20 rounded-xl hover:bg-cyan-500/10 hover:border-cyan-500/30 text-slate-300 hover:text-cyan-400 transition-all" title="إعادة تحميل النظام">
                  <RefreshCw className="w-5 h-5" />
              </button>
          </div>
      </header>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          📊 System Stats Dashboard - Neon Glass Cards
          ═══════════════════════════════════════════════════════════════ */}
      {systemStats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="glass-card p-5 rounded-2xl border border-cyan-500/20 hover:border-cyan-500/30 transition-all hover:shadow-[0_0_20px_rgba(6,182,212,0.1)]">
                  <div className="text-xs text-slate-400 mb-2">حالة النظام</div>
                  <div className={`text-2xl font-bold font-mono ${
                      settings.systemReady && settings.schoolActive ? 'text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'text-red-400 drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]'
                  }`}>
                      {systemStats.systemStatus}
                  </div>
                  <div className="text-xs text-slate-500 mt-1">وضع التخزين: {systemStats.storageMode === 'cloud' ? 'سحابي' : 'محلي'}</div>
              </div>
              <div className="glass-card p-5 rounded-2xl border border-cyan-500/20 hover:border-cyan-500/30 transition-all hover:shadow-[0_0_20px_rgba(6,182,212,0.1)]">
                  <div className="text-xs text-slate-400 mb-2">إجمالي الطلاب</div>
              ...
              <div className="glass-card p-5 rounded-2xl border border-cyan-500/20 hover:border-cyan-500/30 transition-all hover:shadow-[0_0_20px_rgba(6,182,212,0.1)]">
                  <div className="text-xs text-slate-400 mb-2">المستخدمين</div>
                  <div className="text-2xl font-bold font-mono text-cyan-400 drop-shadow-[0_0_8px_rgba(6,182,212,0.5)]">{systemStats.totalUsers}</div>
                  <div className="text-xs text-slate-500 mt-1">مخالفات: {systemStats.totalViolations}</div>
              </div>
              <div className="glass-card p-5 rounded-2xl border border-cyan-500/20 hover:border-cyan-500/30 transition-all hover:shadow-[0_0_20px_rgba(6,182,212,0.1)]">
                  <div className="text-xs text-slate-400 mb-2">الاستئذان اليوم</div>
                  <div className="text-2xl font-bold font-mono text-amber-400 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]">{systemStats.todayExits}</div>
          ...
          <div className="lg:col-span-1 space-y-6">
              ...
              <div className="glass-card p-6 rounded-[2rem] border border-white/10">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                          <label className="text-xs text-gray-400 flex items-center gap-2">
                            <UserCircle className="w-3 h-3 text-blue-400" /> اسم مدير المدرسة
                          </label>
                          <input 
                            type="text" 
                            className="w-full input-glass p-3 rounded-xl text-sm" 
